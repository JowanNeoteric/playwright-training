...
test.describe("Demo QA - elements page", () => {
  ...
  test("opens Text Box and verifies elements", async ({ page }) => {
    ...
    await expect(page.locator(selector.textBox.username)).toHaveText(common.text.textBox.fullName);
    ...
    await expect(page.locator(selector.textBox.email)).toHaveText(common.text.textBox.email);
    ...
    await expect(page.locator(selector.textBox.currentAddress)).toHaveText(common.text.textBox.currentAddress);
    ...
    await expect(page.locator(selector.textBox.permanentAddress)).toHaveText(common.text.textBox.permanentAddress);
    await expect(page.locator(selector.generic.button)).toHaveText(common.text.textBox.submit);
  });
  ...
  test("opens Broken Links and verifies elements", async ({ page }) => {
    ...
    await expect(hyperLink.nth(2)).toHaveAttribute("href", common.url.valid);
    await expect(title.nth(3)).toHaveText("Broken Link");
    await expect(hyperLink.nth(3)).toHaveText("Click Here for Broken Link");
    await expect(hyperLink.nth(3)).toHaveAttribute("href", common.url.invalid);
  });
  ...
  test("opens Upload and Download and verifies elements", async ({ page }) => {
    ...
    const downloadButton = page.locator(selector.generic.button);
  ...
  test("opens Dynamic Properties and verifies elements", async ({ page }) => {
    const dynamicPropertiesMenu = page.getByText(common.text.sections.dynamicProperties);
    const textWithDynamicId = page.locator('p');
    const button = page.locator(selector.generic.button);

    await expect(dynamicPropertiesMenu).toHaveText(common.text.sections.dynamicProperties);
